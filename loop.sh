...
TERMINAL_WINDOW_ID=0x0380000a
DELAY_IN_SWITCHING_WINDOWS_SECONDS=20

while true;
do
	window_id=$(wmctrl -l | cut -f1 -d ' ')
	for i in $(echo "$window_id" | tr " " "\n")
	do
          if [ "$i" != "$TERMINAL_WINDOW_ID" ]
	  then
	  	wmctrl -i -a "$i" ;sleep "$DELAY_IN_SWITCHING_WINDOWS_SECONDS";
		#echo "$i"
	  fi
	done
done
